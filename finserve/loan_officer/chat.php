...
// ✅ Identify Chat Partner for Loan Officer
if ($current_role_id == 7) { // Loan Officer → Branch Manager
    $stmt = $pdo->prepare("SELECT id FROM users WHERE role_id = 5 LIMIT 1"); // Branch Manager role_id = 5
} else {
    die("<p style='text-align:center;color:red;'>Access Denied! Only Loan Officer can access this chat.</p>");
}
$stmt->execute();
...
<head>
<meta charset="UTF-8">
<title>Chat | Loan Officer & Branch Manager</title>
...
<div class="chat-container">
    <h3 style="text-align:center;color:#333;margin-bottom:15px;">
        Chat between Loan Officer and Branch Manager
    </h3>
