...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Fetch user with role_id = 6 (Account Officer)
    $stmt = $pdo->prepare('
        SELECT u.*, r.role_name 
        FROM users u 
        JOIN roles r ON u.role_id = r.id 
        WHERE username = ? AND u.role_id = 6 
        LIMIT 1
    ');
    ...
    if ($user) {
        ...
        $error = 'Sorry, You Have Not Registered Yet. Please Register First.';
    }
}
?>
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        html, body { height: 100%; width: 100%; display: flex; flex-direction: column; background: linear-gradient(135deg, #4e54c8, #8f94fb); }
        ...
        .login-section { flex: 1; padding: 50px; display: flex; flex-direction: column; justify-content: center; background: rgba(255, 255, 255, 0.08); border-radius: 0 15px 15px 0; backdrop-filter: blur(20px); box-shadow: inset 0 0 20px rgba(0, 0, 0, 0.2); }
        ...
        .error { background-color: rgba(255, 0, 0, 0.8); color: yellow; border: 2px solid #ff0000; border-radius: 8px; padding: 12px 18px; text-align: center; font-weight: bold; margin-bottom: 20px; box-shadow: 0 0 15px rgba(255, 0, 0, 0.6); }
        ...
        .logo-circle { width:140px; height:140px; border-radius:50%; background: #fff url('../assets/logo.png') center center/cover no-repeat; box-shadow:0 8px 30px rgba(0,0,0,0.4); margin-bottom:25px; animation:bounce 2s infinite; display:flex; justify-content:center; align-items:center; }
