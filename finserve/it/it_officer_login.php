...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Fetch user with role_id = 9 (IT Officer)
    $stmt = $pdo->prepare('
        SELECT u.*, r.role_name 
        FROM users u 
        JOIN roles r ON u.role_id = r.id 
        WHERE username = ? AND u.role_id = 9
        LIMIT 1
    ');
    ...
    if ($user) {
        ...
        if ($user) {
    // Hash + plain দুইটা situation handle করবে
    if ($password === $user['password'] || password_verify($password, $user['password'])) {
        $_SESSION['user'] = $user;
        header('Location: /finserve/it/dashboard.php');
        exit;
    } else {
...
<head>
    <meta charset="UTF-8">
    <title>IT Officer Login - Finserve</title>
