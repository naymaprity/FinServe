...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Fetch user with role_id = 3
    $stmt = $pdo->prepare('
        SELECT u.*, r.role_name 
        FROM users u 
        JOIN roles r ON u.role_id = r.id 
        WHERE username = ? AND u.role_id = 3
        LIMIT 1
    ');
    ...
    if ($user) {
        ...
        if (password_verify($password, $user['password'])) {
            $_SESSION['user'] = $user;
            header('Location: /finserve/dmd/dashboard.php');
            exit;
        } else {
...
<head>
    <meta charset="UTF-8">
    <title>Deputy Managing Director Login - Finserve</title>
