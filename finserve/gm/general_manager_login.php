...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Fetch user with role_id = 4
    $stmt = $pdo->prepare('
        SELECT u.*, r.role_name 
        FROM users u 
        JOIN roles r ON u.role_id = r.id 
        WHERE username = ? AND u.role_id = 4
        LIMIT 1
    ');
    ...
    if ($user) {
        ...
        if (password_verify($password, $user['password'])) {
            $_SESSION['user'] = $user;
            header('Location: /finserve/gm/dashboard.php');
            exit;
        } else {
...
<head>
    <meta charset="UTF-8">
    <title>General Manager Login - Finserve</title>
